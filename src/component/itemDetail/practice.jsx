...
const Modify = (props) => {
  ...
  /*  if (editedContent.includes("http://localhost:8080/api/img/s3/posts/upload")) {
    setPostContent(urlImg + " " + editedContent);
  } else {
    setPostContent(editedContent);
  } */

  if (
    urlImg.includes("https://blog-img-store2.s3.ap-northeast-2.amazonaws.com")
  ) {
    console.log("urlImg가 있음");
    setPostContent(urlImg + " " + editedContent);
  } else {
    setPostContent(editedContent);
    console.log("urlImg가 없음");
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <span>
        {editedContent} {editedTitle} postContent: {postContent}
      </span>
    </div>
  );
};
